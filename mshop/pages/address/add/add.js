Page({
  ...
  inputProvince(e) { this.setData({ 'form.province': e.detail.value }) },
  inputCity(e) { this.setData({ 'form.city': e.detail.value }) },
  inputDistrict(e) { this.setData({ 'form.district': e.detail.value }) },
  inputDetail(e) { this.setData({ 'form.detail': e.detail.value }) },
  switchDefault(e) { this.setData({ 'form.isDefault': e.detail.value }) },
  ...
  saveAddress() {
    ...
    if (!province || !city || !district || !detail) return wx.showToast({ title: '请填写完整地址', icon: 'none' })
    ...
    if (id) {
      list = list.map(item => item.id == id ? form : item)
    } else {
